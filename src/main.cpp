...
int main(int, char *argv[])
{
    ...
    //  "animation_loop" (This call is different when we compile in standard
    //  mode with GLFW, than when we compile with emscripten to output the
    //  result in a webpage.)
...
void animation_loop()
{
    ...
    if (fps_record.event)
    {
        std::string const title =
            "CGP Display - " + str(fps_record.fps) + " fps";
...
void initialize_default_shaders()
{
    ...
    triangles_drawable::default_texture.initialize_texture_2d_on_gpu(
        white_image);
...
void display_gui_default()
{
    ...
    if (ImGui::CollapsingHeader("Window"))
    {
        ...
        // frequency
        //   vsync may or may not be enforced by your GPU driver and OS (on top
        //   of the GLFW request). de-activating vsync may generate arbitrary
        //   large FPS depending on your GPU and scene.
